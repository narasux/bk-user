...
class TenantUserListToUserInfosMixin:
    ...
    def _get_leader_map(data_source_user_ids: List[int]) -> Dict[Tuple[str, int], List[Dict[str, Any]]]:
        ...
        """
        # 数据源用户 Leader 关系查询
        relations = DataSourceUserLeaderRelation.objects.filter(
            user_id__in=data_source_user_ids,
        ).only("user_id", "leader_id")
        ...
        leaders = TenantUser.objects.filter(
            status=TenantUserStatus.ENABLED, data_source_user_id__in=leader_ids
    ...
    def _get_department_map(data_source_user_ids: List[int]) -> Dict[Tuple[str, int], List[Dict[str, Any]]]:
        ...
        tenant_departments = TenantDepartment.objects.filter(
            status=TenantDepartmentStatus.ENABLED, data_source_department_id__in=department_ids
...
class ProfileListApi(LegacyOpenApiCommonMixin, generics.ListAPIView, TenantUserListToUserInfosMixin):
    ...
    def _filter_queryset(self, params: Dict[str, Any]) -> QuerySet[TenantUser]:
        ...
        if not params.get("include_disabled"):
            queryset = queryset.filter(status=TenantUserStatus.ENABLED)
...
class ProfileRetrieveApi(LegacyOpenApiCommonMixin, generics.RetrieveAPIView):
    ...
    def _get_leaders(tenant_user: TenantUser) -> List[Dict[str, Any]]:
        ...
        return [
            {
                "id": ld.data_source_user.id,
                "username": ld.id,
                "display_name": TenantUserHandler().generate_tenant_user_display_name(ld),
            }
            for ld in leaders
        ]
    ...
    def _build_user_info(self, tenant_user: TenantUser, fields: List[str]) -> Dict[str, Any]:
        ...
        user_info = {
            ...
            "category_id": tenant_user.data_source_id,
            # TODO 1. 支持软删除后需要特殊处理 2. 支持状态时需要特殊处理
            # TODO (su) 讨论新旧状态如何转换？staff_status 迁移逻辑？
            "status": "",
            "staff_status": "",
            "enabled": True,
            # TODO: 协同时需要调整为按照协同租户配置的用户自定义字段进行输出
...
class DepartmentProfileListApi(LegacyOpenApiCommonMixin, generics.ListAPIView, TenantUserListToUserInfosMixin):
    ...
    def get(self, request, *args, **kwargs):
        ...
        tenant_dept = TenantDepartment.objects.filter(id=kwargs["id"], status__in=statuses).first()
        if not tenant_dept:
    ...
    def _filter_queryset(tenant_dept: TenantDepartment, recursive: bool) -> QuerySet[TenantUser]:
        ...
        return TenantUser.objects.filter(
            tenant_id=tenant_dept.tenant_id, status=TenantUserStatus.ENABLED, data_source_user_id__in=user_ids
        ).select_related("data_source_user")
...
class ProfileLanguageUpdateApi(ExcludePatchAPIViewMixin, LegacyOpenApiCommonMixin, generics.UpdateAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        tenant_user = TenantUser.objects.filter(id=kwargs["username"], status=TenantUserStatus.ENABLED).first()
        if not tenant_user:
            raise Http404(f"user username:{kwargs['username']} not found or disabled")
